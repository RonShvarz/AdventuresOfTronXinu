...
void init_enemy_lvl4()
{
	...
	*/
	...
	for (j = 0; j < 6; j++)
	{
		...
		enemy.position.y = rand() % FLOORS_LENGTH + 1;
		for (i = 0; i < 2; i++)
			enemy.blocks[i].attribute = 66;
		allEnemy[j] = enemy;
	}
...
int Int0x70Handler(int mdevno)
{
	...
	}//Movement for the floating bits
	 /*if (count0x70 % (RTC_RATE / 8) == 0)
	 {
	 for (i = 0; i < FBITS; i++)
	 if (fbits[i].active)
	...
	{ // eat all floating bits -> only the center line will be blue until the 'Tron' come to center
		{
			...
			initLadder(&ladder);
			blueNow = FALSE;
			if (tron.position.x == 40 || tron.position.x == 40) {
...
xmain()
{
	...
	schedule(3, 2, dispid, 0, uppid, 1);//(Number of pids, number of cycles, the pid and it's insertion...), manage the rotation between them
}//xmain